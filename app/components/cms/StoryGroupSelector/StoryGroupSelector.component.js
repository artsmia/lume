...
export default class StoryGroupSelector extends Component {
  ...
  render() {
    ...
    return (
      <Container my={2} p={2} flexWrap={'wrap'}>
        <Box w={1}>
          <H2>Associated Groups</H2>
        </Box>

        {categories.map(category => (
          <Flex key={category.id} w={1} flexWrap={'wrap'} pl={2}>
            <Box w={1}>
            ...
            <Flex w={1} flexWrap={'wrap'}>
              {category.groups.map(group => (
                <Flex key={group.id} w={1} justifyContent={'flex-start'}>
                  <CheckboxInput
                    value={group.id}
