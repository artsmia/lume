  ...
  render() {
    ...
    return (
      <ExpanderContainer
        ...
        id={this.props.id}
      >
        <Header w={1} alignItems={'center'}>
          <A onClick={open ? handleCloseRequest : handleOpenRequest}>
            {icon ? (
              icon
            ) : (
              <Icon icon={open ? 'keyboard_arrow_down' : 'keyboard_arrow_up'} />
            )}
          </A>
